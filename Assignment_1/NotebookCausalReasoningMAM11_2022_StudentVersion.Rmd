...
```{r}
#Checking the overlap between the propensity scores for the two groups
...
par(mfrow=c(2,1))
hist(gotTreatment2, ylim=c(0,650), col='darkblue')
hist(gotNoTreatment2, ylim=c(650,0), col='red')
```
```{r}
pscoreRF_v <- c(pscoreRF[,1])   #creating a vector
```
...
```{r}
#############################################################################################
#2. Compare the results to those previously obtained by matching

#comparing data from nearest neighbor matching with previous data
...
#Anwer also taking into account what written here:
