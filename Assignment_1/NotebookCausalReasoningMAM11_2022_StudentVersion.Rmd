...
# ------- solution to be optimized later on ---------     ################### CHECK IT LATER ########################
...
#density hist
par(mfrow=c(2,1))
hist(gotTreatment, main='Group with treatment', xlab='Propensity Score', ylim=c(0,2), col='darkblue', 
     prob = TRUE)   #shows density instead of frequency
lines(density(gotTreatment),
      lwd = 2,
      col = "orange")

hist(gotNoTreatment, main='Group without treatment', xlab='Propensity Score', ylim=c(4,0), col='red', 
     prob = TRUE)   #shows density instead of frequency
...
print("This overlap is almost everywhere. Although there is poor overlap on the right tail, the poritivity assumption is still reasonable.")   
...
#density hist
par(mfrow=c(2,1))
hist(logitTreatment, main='Group with treatment', xlab='Log-odds of Propensity Score', xlim=c(-5,5), ylim=c(0,0.6), col='darkblue', 
     prob = TRUE)   #shows density instead of frequency
lines(density(logitTreatment),
      lwd = 2,
      col = "orange")

hist(logitNoTreatment, main='Group without treatment', xlab='Log-odds of Propensity Score', xlim=c(-5,5),ylim=c(0.6,0), col='red', 
     prob = TRUE)   #shows density instead of frequency
...
#What do you think about the overlap?
print("There is good overlap almost everywhere so the positivity assumption is reasonable.")     
...
print("The means before matching was 0.0893, While after it is 0.0623, so it is now different and smaller. ")          

################### CHECK THIS, not sure if we should add something else #################
...
print("The odds ratio of treatment is 1.315")          ################### CHECK THIS #################        
```
...
print("The odds ratio of treatment is now 1.339.")        ######### CHECK THIS #################      
print("===================================================")
```
...
#density hist
par(mfrow=c(2,1))
hist(gotTreatment2, main='Group with treatment', xlab='Propensity Score', xlim=c(0,1), ylim=c(0,3), col='darkblue', 
     prob = TRUE)   #shows density instead of frequency
lines(density(gotTreatment2),
      lwd = 2,
      col = "orange")

hist(gotNoTreatment2, main='Group without treatment', xlab='Propensity Score', xlim=c(0,1),ylim=c(3,0), col='red', 
     prob = TRUE)   #shows density instead of frequency
...
matched2 <- rhc.selected[unlist(psmatch[c("index.treated","index.control")]), ]
```
...
print('When comparing the results obtain previously with the one with a random forest method, we can see that we obtain poor overlap for high propensity scores and we only have subjects without treatment.')
...
print("We prefer the approach that uses logistic regression for different reasons:
      1. It relays on functional form assumption;
      2. We got better results in terms of propensity score;
      3. It is more used so more documentation can be found.")         ######### CHECK THIS ################# 
